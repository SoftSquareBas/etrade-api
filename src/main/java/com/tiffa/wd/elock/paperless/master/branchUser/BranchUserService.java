...
public class BranchUserService {
	...
	public GridData search(final BranchUserModel model) {
		...
		sql.append(" 		, (CASE WHEN u.user_id = b.user_id THEN 1 ELSE 0 END) AS \"isSecureUser\" ");
		...
		return coreRepository.searchPagingGridData(sql.toString(), params, sort, conversion);
	}

	public Data load(final BranchUserModel model) {
	...
	public GridData loadAccessRight(final BranchUserModel model) {
		SqlParams params = SqlParams.create();
		StringBuilder sql = new StringBuilder();
		...
		params.add("userId", model.getUserId());
		params.add("companyUserId", SecurityUtils.getUserId());
		params.add("companyId", model.getCompanyId());
		params.add("branchId", model.getBranchId());
	...
	public Data validate(final BranchUserModel model) {
	...
	private void saveAccesRight(final BranchUserModel model) {
		for (BranchUserModel.TransferItem item : model.getItems()) {
	...
	@Transactional
	public Data add(final BranchUserModel model) {
	...
	@Transactional
	public Data edit(final BranchUserModel model) {
		ValidationUtils.checkRequired(model.getCompanyId(), "Company Code");
		ValidationUtils.checkRequired(model.getBranchId(), "Branch Code");
		ValidationUtils.checkRequired(model.getUsername(), "Username");
		ValidationUtils.checkRequired(model.getPassword(), "Password");
	...
	@Transactional
	public Data delete(final BranchUserModel model) {
